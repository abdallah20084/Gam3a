...
// Define the Socket.IO server URL from environment variable
const SOCKET_SERVER_URL = process.env.NEXT_PUBLIC_SOCKET_SERVER_URL || 'http://localhost:3000';

// Type Definitions for Socket.IO events (Client-side)
...
export default function GroupChatPage() {
  ...
  useEffect(() => {
    ...
    const onConnect = () => {
      console.log('Connected to Socket.IO server from group page.');
      setIsConnected(true);
    ...
    const onDisconnect = () => {
      console.log('Disconnected from Socket.IO server from group page.');
      setIsConnected(false);
      setTypingUsers(new Set()); 
    };

    const onJoinedGroup = (data: { groupId: string; messages: Message[] }) => {
      console.log(`Successfully joined group room: ${data.groupId}`);
      setMessages(data.messages.map(msg => ({
          id: msg.id,
          groupId: msg.groupId,
          sender: msg.sender, 
          // <--- FIX: Sanitize content here before it's stored and rendered
    ...
    const onUserStatusUpdate = (data: { userId: string; isOnline: boolean }) => {
        console.log(`User status update: ${data.userId} is ${data.isOnline ? 'online' : 'offline'}`);
        setOnlineUsers(prev => {
            const newSet = new Set(prev);
            if (data.isOnline) {
                newSet.add(data.userId);
  ...
  const handleSendMessage = useCallback((e: React.FormEvent) => {
    ...
    // <--- FIX: Sanitize message content before sending it
    const sanitizedContent = DOMPurify.sanitize(newMessage.trim());

    socketRef.current.emit('sendMessage', {
      groupId: group.id,
      content: sanitizedContent, // <--- استخدم المحتوى المنقى
      token: token,
    });
  ...
  const startEditing = useCallback((message: Message) => {
    ...
    // <--- FIX: Sanitize content before populating edit field
  ...
  const submitEdit = useCallback(() => {
    ...
    // <--- FIX: Sanitize edited content before sending it
    const sanitizedContent = DOMPurify.sanitize(editingContent.trim());

    socketRef.current.emit('editMessage', {
      messageId: editingMessageId,
      groupId: group.id,
      newContent: sanitizedContent, // <--- استخدم المحتوى المنقى
      token: token,
    });
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <main className="flex-1 p-6 container mx-auto flex">
        ...
        <div className="flex-1 bg-white rounded-lg shadow-md p-6 mr-4 flex flex-col">
          ...
          {/* Tab Navigation (Placeholder for future functionality) */}
                      ...
                      {!isSystemMessage ? (
                          <>
                              ...
                              {editingMessageId === msg.id ? (
                                  ...
                                  // <--- FIX: Use dangerouslySetInnerHTML for sanitized HTML
                          ...
                          // System Message Content - also sanitize before rendering
        ...
        <div className="w-1/4 bg-white rounded-lg shadow-md p-6">
          ...
          {group.coverImageUrl && (
            <img src={group.coverImageUrl} alt={group.name} className="w-full h-32 object-cover rounded-lg mb-4" />
          )}
