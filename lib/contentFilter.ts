...
export class ContentFilter {
  ...
  private status: 'idle' | 'loading' | 'loaded' | 'fallback' | 'error' = 'idle';
  ...
  async checkImage(imageElement: HTMLImageElement): Promise<ContentFilterResult> {
    if (!this.isLoaded) {
    ...
    try {
      ...
      const dominantCategory = predictions.reduce((prev, current) => 
  ...
  private fallbackImageCheck(imageElement: HTMLImageElement): ContentFilterResult {
    ...
    try {
      ...
      // حساب درجة الخطورة
      let riskScore = 0;
      if (isTooRed) riskScore += 0.3;
      if (hasHighSkinTone) riskScore += 0.2;
      if (hasHighDarkRatio) riskScore += 0.2;
      if (isTooDark) riskScore += 0.1;
      if (isTooBright) riskScore += 0.1;
  ...
  async checkImageFromFile(file: File): Promise<ContentFilterResult> {
  ...
  async checkVideoThumbnail(videoFile: File, time: number = 1): Promise<ContentFilterResult> {
    return new Promise((resolve, reject) => {
  ...
  async checkMultipleVideoFrames(videoFile: File, frameCount: number = 5): Promise<ContentFilterResult[]> {
    return new Promise((resolve, reject) => {
      const video = document.createElement('video');
  ...
  async isVideoSafe(videoFile: File, frameCount: number = 5): Promise<{
    isSafe: boolean;
    confidence: number;
    frameResults: ContentFilterResult[];
  }> {
    try {
  ...
  private async fallbackVideoCheck(videoFile: File): Promise<{
    isSafe: boolean;
    confidence: number;
    frameResults: ContentFilterResult[];
  }> {
    ...
    return new Promise((resolve, reject) => {
      ...
      video.onloadedmetadata = () => {
        try {
          ...
          // حساب درجة الخطورة
          let riskScore = 0;
          if (isLargeFile) riskScore += 0.2;
          if (isVeryShort) riskScore += 0.3;
          if (isVeryLong) riskScore += 0.1;
          if (hasSuspiciousExtension) riskScore += 0.1;
          
          const isSafe = riskScore < 0.4;
          
          // إنشاء نتيجة وهمية للإطار
          const frameResult: ContentFilterResult = {
            isSafe,
